...
```shell
dotnet new install Amantinband.Evently.Template

dotnet new clean-arch -o Evently
```
...
1. Deleting the subscription from the database and marking all reminders as deleted ([Subscriptions/Events/SubscriptionDeletedEventHandler.cs](src/Evently.Application/Subscriptions/Events/SubscriptionCanceledEventHandler.cs)])
1. Deleting all the reminders marked as deleted from the database ([Reminders/Events/ReminderDeletedEventHandler.cs](src/Evently.Application/Reminders/Events/ReminderDeletedEventHandler.cs)]
...
1. If `domain object B` needs to react to changes in `domain object A`, a [Domain Event](src/Evently.Domain/Common/IDomainEvent.cs) is added to `domain object A` alongside the changes.
1. Upon persisting `domain object A` changes to the database, the domain events are [extracted and added to a queue](src/Evently.Infrastructure/Common/Persistence/AppDbContext.cs) for offline processing:
    ```csharp
    ...
    ```
1. After the user receives a response, the [EventualConsistencyMiddleware](src/Evently.Infrastructure/Common/Middleware/EventualConsistencyMiddleware.cs) is invoked and processes the domain events:
    ```csharp
...
There is a simple background service that runs every minute and sends email reminders for all reminders that are due ([ReminderEmailBackgroundService](src/Evently.Infrastructure/Reminders/BackgroundServices/ReminderEmailBackgroundService.cs)):
...
```shell
dotnet user-secrets --project src/Evently.Api set EmailSettings:EnableEmailNotifications true
dotnet user-secrets --project src/Evently.Api set EmailSettings:DefaultFromEmail amantinband@gmail.com
dotnet user-secrets --project src/Evently.Api set EmailSettings:SmtpSettings:Server smtp-relay.brevo.com
dotnet user-secrets --project src/Evently.Api set EmailSettings:SmtpSettings:Port 587
dotnet user-secrets --project src/Evently.Api set EmailSettings:SmtpSettings:Username amantinband@gmail.com
dotnet user-secrets --project src/Evently.Api set EmailSettings:SmtpSettings:Password your-password
```
...
- [Evently](https://github.com/jasontaylordev/Evently) - An awesome clean architecture solution template by Jason Taylor
